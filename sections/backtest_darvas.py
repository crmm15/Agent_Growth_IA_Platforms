...
def robust_trend_filter(df):
    """
    Filtro de tendencia robusto para primeras velas:
    - Tendencia alcista si Close > mavilimw
    - Para primeras señales, verifica las últimas velas.
    """
...
def backtest_darvas():
    ...
    if st.button("Ejecutar backtest"):
        # Carga de datos
        df = yf.Ticker(activo_nombre).history(period=timeframe)
        if df.empty:
        ...
        ax.plot(df.index, df['Close'], label='Precio', zorder=1)
        ...
        ax.scatter(df.index[df['buy_final']], df.loc[df['buy_final'],'Close'], label='Entrada', marker='^', zorder=3)
        ax.scatter(df.index[df['sell_signal']], df.loc[df['sell_signal'],'Close'], label='Salida', marker='v', zorder=3)
