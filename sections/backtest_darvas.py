...
def backtest_darvas():
    ...
    # 3) Filtro de tendencia con MavilimW (pendiente de la propia MavilimW)
    df['mavilimw']    = calc_mavilimw(df)
    # Lógica: tendencia alcista cuando MAVW sube, bajista cuando baja
    ...
    # Calculamos momentum bajista por simetría con WAE
    ...
    # 5) Señales finales (solo si confluyen las tres condiciones)
    df['buy_final']  = df['buy_signal']  & df['trend_up']   & df['wae_filter_buy']
    df['sell_final'] = df['sell_signal'] & df['trend_down'] & df['wae_filter_sell']

    # 6) Solo la primera señal de cada tipo
    if df['buy_final'].any():
        fb = df['buy_final'].idxmax()
        df.loc[:, 'buy_final'] = False
        df.at[fb, 'buy_final']  = True
    if df['sell_final'].any():
        fs = df['sell_final'].idxmax()
        df.loc[:, 'sell_final']= False
        df.at[fs, 'sell_final'] = True

    # 7) Mostrar tabla de señales
    cols = [
        'Date', 'Close',
        'darvas_high','darvas_low',
        'mavilimw',
    ...
    ax.scatter(buy_idx,  df.loc[df['buy_final'],  'Close'],
               marker="^", color="green", s=100, label="Señal Compra", zorder=3)
    ax.scatter(sell_idx, df.loc[df['sell_final'], 'Close'],
