...
def backtest_darvas():
    ...
    # Buffer para garantizar históricos suficientes
    buffer_days  = DARVAS_WINDOW + SLOW_EMA + 2
    start_buffer = pd.to_datetime(start) - pd.Timedelta(days=buffer_days)

    st.info("Descargando datos históricos...")
    df = cargar_precio_historico(activo, timeframe, start_buffer, end)
    ...
    # 3) Filtro de tendencia con MavilimW (shift 2)
    ...
    # 6) Filtrar al rango UI y mostrar tabla de señales
